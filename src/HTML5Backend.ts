...
export default class HTML5Backend implements Backend {
	...
	private dragStartSource: ISource[] | null = null
	...
	public connectDragSource(sourceId: string, node: any, options: any) {
		...
		// 以闭包的方式保存source，将粒度控制到connectSource上
		const source = {sourceId, options, node}

		const handleDragStart = (e: any) => this.handleDragStart(e, source)
	...
	private handleDragStart(e: any, source: any) {
		if (!this.dragStartSource) {
			this.dragStartSource = []
		}
		this.dragStartSource.unshift(source)
	}

	@autobind
	private handleTopDragStart(e: any) {
		const { dragStartSource } = this
		if (dragStartSource && dragStartSource.length === 0) return;
		this.dragStartSource = null
		...
		if (this.monitor.isDragging()) {
			if (typeof dataTransfer.setDragImage === 'function') {
				...
				const source = (dragStartSource as ISource[]).find(source => source.sourceId === sourceId)

				// const sourceNode = this.sourceNodes.get(sourceId)
				const sourceNode = source ? source.node : this.sourceNodes.get(sourceId);
	...
	private handleTopDrop(e: any) {
		...
		if (dropTargetIds.length === 0) return;
		this.dropTargetIds = []
